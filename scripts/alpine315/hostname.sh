...
if [[ "$PACKER_BUILD_NAME" =~ ^magma-alpine315-(vmware|hyperv|libvirt|parallels|virtualbox)$ ]]; then
  printf "\n127.0.0.1    magma magma.localdomain\n" >> /etc/hosts
  echo "magma.localdomain" > /etc/hostname
  hostname magma.localdomain
elif [[ "$PACKER_BUILD_NAME" =~ ^generic-alpine315-(vmware|hyperv|libvirt|parallels|virtualbox)$ ]]; then
