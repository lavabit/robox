...
if [[ "$PACKER_BUILD_NAME" =~ ^magma-openbsd7-(vmware|hyperv|libvirt|parallels|virtualbox)$ ]]; then
  printf "\n127.0.0.1    magma magma.localdomain\n" >> /etc/hosts
  echo "magma.localdomain" > /etc/myname
  hostname magma.localdomain
elif [[ "$PACKER_BUILD_NAME" =~ ^generic-openbsd7-(vmware|hyperv|libvirt|parallels|virtualbox)$ ]]; then
