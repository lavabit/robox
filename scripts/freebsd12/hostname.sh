...
if [[ "$PACKER_BUILD_NAME" =~ ^magma-freebsd12-(vmware|hyperv|libvirt|parallels|virtualbox)$ ]]; then
...
elif [[ "$PACKER_BUILD_NAME" =~ ^generic-freebsd12-(vmware|hyperv|libvirt|parallels|virtualbox)$ ]]; then
  sed -i -e "s/hostname=\".*\"/hostname=\"freebsd12.localdomain\"/g" /etc/defaults/rc.conf-e
