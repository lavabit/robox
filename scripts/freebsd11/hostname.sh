...
if [[ "$PACKER_BUILD_NAME" =~ ^magma-freebsd-(vmware|hyperv|libvirt|parallels|virtualbox)$ ]]; then
  sed -i -e "s/hostname=\".*\"/hostname=\"magma.localdomain\"/g" /etc/defaults/rc.conf
  hostname magma.localdomain
elif [[ "$PACKER_BUILD_NAME" =~ ^generic-freebsd-(vmware|hyperv|libvirt|parallels|virtualbox)$ ]]; then
