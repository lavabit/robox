...
if [[ "$PACKER_BUILD_NAME" =~ ^magma-hardenedbsd12-(vmware|hyperv|libvirt|parallels|virtualbox)-(x64|x32|a64|a32|p64|p32|m64|m32)$ ]]; then
...
elif [[ "$PACKER_BUILD_NAME" =~ ^generic-hardenedbsd12-(vmware|hyperv|libvirt|parallels|virtualbox)-(x64|x32|a64|a32|p64|p32|m64|m32)$ ]]; then
  [ -f /etc/defaults/rc.conf-e ] && sed -i -e "s/hostname=\".*\"/hostname=\"hardenedbsd12.localdomain\"/g" /etc/defaults/rc.conf-e
