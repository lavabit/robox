...
if [[ "$PACKER_BUILD_NAME" =~ ^magma-dragonflybsd5-(vmware|hyperv|libvirt|parallels|virtualbox)$ ]]; then
...
elif [[ "$PACKER_BUILD_NAME" =~ ^generic-dragonflybsd5-(vmware|hyperv|libvirt|parallels|virtualbox)$ ]]; then
  sed -i -e "s/hostname=\".*\"/hostname=\"dragonflybsd5.localdomain\"/g" /etc/defaults/rc.conf-e
