...
if [[ "$PACKER_BUILD_NAME" =~ ^generic-oracle9-(vmware|hyperv|docker|libvirt|parallels|virtualbox)-(x64|x32|a64|a32|p64|p32|m64|m32)$ ]]; then
  printf "oracle9.localdomain\n" > /etc/hostname
  printf "\n127.0.0.1 oracle9.localdomain\n\n" >> /etc/hosts
else
