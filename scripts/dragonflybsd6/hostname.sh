...
if [[ "$PACKER_BUILD_NAME" =~ ^magma-dragonflybsd6-(vmware|hyperv|libvirt|parallels|virtualbox)-(x64|x32|a64|a32|p64|p32|m64|m32)$ ]]; then
...
elif [[ "$PACKER_BUILD_NAME" =~ ^generic-dragonflybsd6-(vmware|hyperv|libvirt|parallels|virtualbox)-(x64|x32|a64|a32|p64|p32|m64|m32)$ ]]; then
