...
if [[ "$PACKER_BUILD_NAME" =~ ^generic-centos8s-(vmware|hyperv|libvirt|parallels|virtualbox)-(x64|x32|a64|a32|p64|p32|m64|m32)$ ]]; then
  printf "centos8s.localdomain\n" > /etc/hostname
  printf "\n127.0.0.1 centos8s.localdomain\n\n" >> /etc/hosts
else
