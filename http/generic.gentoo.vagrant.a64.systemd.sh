...
# Download the current-stage3-arm64-systemd and the portage tarballs, unpack them, and then delete the archive files.m
echo 'Downloading Image Overlay'
# host="https://gentoo.osuosl.org"
host="https://mirrors.kernel.org/gentoo"
tarball=$(wget -q $host/releases/arm64/autobuilds/current-stage3-arm64-systemd/ -O - | grep -E -v "tar\.xz\.asc|tar\.xz\.CONTENTS\.gz|tar\.xz\.DIGESTS|tar\.xz\.sha256" | grep -E -o -e "stage3-arm64-systemd-[0-9]{8}T[0-9]{6}Z.tar.xz" | sort -V | uniq | tail -1)
wget --tries=5 --progress=dot:binary $host/releases/arm64/autobuilds/current-stage3-arm64-systemd/$tarball || exit 1
wget --tries=5 -q $host/releases/arm64/autobuilds/current-stage3-arm64-systemd/$tarball.asc || exit 1
wget --tries=5 -q $host/releases/arm64/autobuilds/current-stage3-arm64-systemd/$tarball.DIGESTS || exit 1
...
chroot /mnt/gentoo /bin/bash < /root/generic.gentoo.vagrant.chroot.a64.systemd.sh
